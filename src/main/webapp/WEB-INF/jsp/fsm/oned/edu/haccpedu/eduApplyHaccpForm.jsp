    ...
    <div id="container">
        <div class="sub_content">
            <form id="applyFrm" onsubmit="return false">
                ...
                <input type="hidden" id="GBN" value="<c:out value="${paramMap.GBN}"/>"/><!-- 친환경 1 HACCP 2-->
                ...
                <div class="table_popup">
                    ...
                    <table class="table_col responsive mt5 mb20">
                        ...
                        <tr>
                            <th>업체명 <em>*</em></th>
                            <td>
                            	<input type="text" id="COMP_NM_KOR" value="<c:out value="${compData.COMP_NM_KOR}"/>" placeholder="">
                            </td>
                            <th>등록번호<br/>(미등록 시설은 제외)</th>
                            <td>
                                <input type="text" id="CERT_NO" <c:if test="${paramMap.STATE == 'R'}">value="<c:out value="${applyData.CERT_NO}"/>" disabled</c:if> placeholder="예) AQ-0001">
                            </td>
                        </tr>
                        <tr>
                            <th>대표자 <em>*</em></th>
                            <td>
                            	<input type="text" id="CEO_NM_KOR" value="<c:out value="${compData.CEO_NM_KOR}"/>" placeholder="">
                            </td>
                            <th>사업자등록번호</th>
                            <td>
                            	<input type="text" id="BIZ_REG_NO" value="<c:out value="${compData.BIZ_REG_NO}"/>" placeholder="-없이 10자리 입력">
                            </td>
                        </tr>
                        <tr>
                            <th>시설(양식장) 소재지</th>
                            <td colspan="3">
                                <c:if test="${paramMap.STATE == 'I'}">
                                    <%--<span class="unit-none">
                                        <c:out value="(${compData.COMP_ZIPCD_ST})${compData.COMP_ADDR1_ST}${compData.COMP_ADDR2_ST}"/>
                                    </span>--%>
                ...
                <!-- signature -->
                <div class="md_apply">
                    <div class="text">생산·출하 전 단계 안전관리인증기준(HACCP) 교육을 신청합니다.</div>
                        ...
                        <tr>
                            <td>
                                ...
                                <table>
                                    ...
                                    <tr>
                                        <th>수집·이용 목적</th>
                                        <td>생산·출하 전 단계 HACCP 교육 이수관리</td>
                                    </tr>
                                    <tr>
                                        <th>수집·이용 항목</th>
                                ...
                                <div class="md_end">
                                    ...
                                    <span>- 친환경수산물 등 인증사업자 의무교육 이수와 관련된 개인정보의 수집·이용에 동의합니다.</span>
                                    <div class="ckd_wrap">
...
<script>
    ...
    $(document).ready(function() {
        ...
        lpCom.setCodeSelect(["EDU_APLY_UNIT"], {MSTR_CD:["FS_542"],first:"S"}); //신청단위
    ...
    var fnSetObjectEvent = function() {
        {//click 이벤트 활당
            ...
            $("#saveBtn").on("click", function(e) {
                ...
                lpCom.Ajax("save", "/edu/saveEduApply.do", param, calBackFunc, otherInit);
            });

            //출력
            $("#printBtn").on("click", function(e) {
                let objReportParam = {};
                objReportParam["connection.reportname"] = "nfpis_report/FSM/ONED/OZ_ONED_03.ozr";   // 폼 디자인명 (경로+파일명)
    ...
    var selectEduApplyWorkOptions = function(pTargetId,pParam){
        pParam.pUrl = "/edu/selectEduApplyHaccpWorkOptions.do?EDU_APLY_UNIT="+$("#EDU_APLY_UNIT").val();
        pParam.cond = {MSTR_CD:["FS_542"]};
        pParam.first = "S";
