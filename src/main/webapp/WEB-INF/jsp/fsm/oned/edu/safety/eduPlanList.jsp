    ...
    var fnSetList = function(pData){
        ...
        for(i in pData.list){
            ...
            if(pData.list[i].EDU_STATUS == "2"){
                listHtml.push('<td data-title="SAFETY 교육 신청" onclick="event.cancelBubble=true"><button type="button" class="sbtn_resi re_btn" onclick=fnMoveEduApplyForm("' + pData.list[i].EDU_ID + '")>교육신청</button></td>');
            }else{
                listHtml.push('');
            }
            listHtml.push('</tr>');
