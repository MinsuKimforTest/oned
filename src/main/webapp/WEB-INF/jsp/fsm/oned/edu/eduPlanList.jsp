...
<script>
    ...
    var fnSearch = function(pSelectPage) {
        ...
        lpCom.Ajax("search", "/edu/selectEduPlanList.do", param, calBackFunc, otherInit);
    }
    ...
    var fnSetList = function(pData){
        ...
        for(i in pData.list){
            listHtml.push('<tr onclick=fnMoveDetail("' + pData.list[i].EDU_ID + '","' + pData.list[i].GBN + '")>');
            ...
            if(pData.list[i].EDU_STATUS == "2"){
                if(pData.list[i].GBN == "1") {
                    listHtml.push('<td data-title="친환경 교육 신청" onclick="event.cancelBubble=true"><button type="button" class="sbtn_resi re_btn" onclick=fnMoveEduApplyForm("' + pData.list[i].EDU_ID + '")>교육신청</button></td>');
                }else{
                    listHtml.push('<td data-title="HACCP 교육 신청" onclick="event.cancelBubble=true"><button type="button" class="sbtn_resi re_btn" onclick=fnMoveEduApplyForm2("' + pData.list[i].EDU_ID + '")>교육신청</button></td>');
                }
            }else{
                listHtml.push('');
            }
    ...
    //상세
    var fnMoveDetail = function(eduId, gbn) {
        lpCom.setSearchCond(); //페이지 이동 검색조건 파라메터 저장(폼 ID는 schFrm 해야 작동함)
        var postData = {};
        postData.returnUrl = "${currentMemu.CURRENT_MENU_URL}";
        lpCom.hrefOrSubmit("/edu/actionEduPlanDetail.do?EDU_ID=" + eduId+"&GBN="+gbn ,postData);
    }
    ...
    var fnMoveEduApplyForm = function(eduId) {
        ...
        if(checkYn == "N"){
            if(confirm("교육신청 시, 친환경 인증번호 확인이 필요합니다.")){
                $("#certDiv").fadeIn();
            }
    ...
    //HACCP 접수 신청
    var fnMoveEduApplyForm2 = function(eduId) {
        var checkYn = $("#checkYn").val();
