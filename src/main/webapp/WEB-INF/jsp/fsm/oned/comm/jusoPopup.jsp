...
<%@page import="fsm.oned.comm.util.Globals" %>
...
<head>
    <%
        ...
        String confmKey = Globals.JUSO_SEARCH_KEY; // 검색API 승인키
        String domain = Globals.JUSO_HOST_CLIENT; // 인터넷망
    ...
    %>
    <script src="<c:url value="/js/comm/jquery-3.5.1.min.js"/>"></script>
    <script src="<c:url value="/juso/js/addrlink.js"/>"></script>
    <link rel='stylesheet' href='<c:url value="/juso/css/addrlink.css"/>'/>
    ...
    <script language="javascript">
        ...
        function setParent() {
            ...
            if(rtDetBdNmList != "") {
            	rtAddrPart1 = rtAddrPart1 + " " + rtDetBdNmList;
            }

            var juso = {
                "roadFullAddr": rtRoadAddr
            ...
            $("#coordForm [name=admCd]").val(rtAdmCd);
            $("#coordForm [name=rnMgtSn]").val(rtRnMgtSn);
            $("#coordForm [name=udrtYn]").val(rtUdrtYn);
            $("#coordForm [name=buldMnnm]").val(rtBuldMnnm);
            $("#coordForm [name=buldSlno]").val(rtBuldSlno);

            $.ajax({
                url: "../comm/getAddrCoordApi.do" 
                , type: "post"
                , data: $("#coordForm").serialize() 							// 요청 변수 설정
                , dataType: "json"											
                , success: function (jsonStr) {									// jsonStr : 주소 검색 결과 JSON 데이터
                	console.log(jsonStr);
                    var errCode = jsonStr.results.results.common.errorCode;
                    var totalCount = jsonStr.results.results.common.totalCount;
					
                    //results 하나 없애기
                     if (errCode == "0" && totalCount != "0") {
                        var entX = jsonStr.results.results.juso[0].entX;
                        var entY = jsonStr.results.results.juso[0].entY;

                        juso.entX = entX;
                        juso.entY = entY;
                        opener.calBackFunc('<c:out value="${param.sid}" />', juso);
                        window.close();
                    } else {
                        opener.calBackFunc('<c:out value="${param.sid}" />', juso);
                        window.close();
        ...
        function setMaping(idx) {
            ...
            if(detBdNmList == "") {
            	...
            	$("#addrPart1").html(addrPart1 + " " + detBdNmList);
            }
        ...
        $(document).ready(function () {
            ...
            $('#roadNameList2').scroll(function () {
                prevPosition = this.scrollTop;
            });
        });
...
<form name="AKCFrm" id="AKCFrm" method="post">
    ...
    <input type="hidden" name="confmKey" id="confmKey" value="<%=confmKey %>"/>
        ...
        <div class="pop-address-search-inner" style="border: 0px;">
            <div style="height: 365px;" id="searchContentBox">
                <fieldset>
                    ...
                    <span class="wrap">
                        <input type="text" class="popSearchInput" style="font-size: 13px; font-weight: bold; ime-mode:active;" title="검색어를 입력하세요" name="keyword" id="keyword"
                               placeholder="도로명주소, 건물명 또는 지번입력" value="" tabindex="1"/>
                ...
                <div class="detail" style="display:none;" id="resultData">
                    ...
                    <table class="data-row">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="rtAddrDetail" id="rtAddrDetail" style="width: 100%; font-size: 13px;" onkeypress="addrDetailChk();" onkeyup="addrDetailChk1(this);"
                                       title="상세주소"/>
...
<form name="coordForm" id="coordForm" method="post">
	<input type="hidden" name="domain" id="domain" value="<%=domain %>"/>
    <input type="hidden" name="confmKey" id="confmKey" value="<%=Globals.JUSO_COORD_SEARCH_KEY%>"/><!-- 요청 변수 설정 (승인키) -->
    <input type="hidden" name="resultType" value="json"/> <!-- 요청 변수 설정 (검색결과형식 설정, json) -->
    <input type="hidden" name="admCd" id="admCd" value=""/> <!-- 요청 변수 설정 (행정구역코드) -->
    <input type="hidden" name="rnMgtSn" id="rnMgtSn" value=""/><!-- 요청 변수 설정 (도로명코드) -->
    <input type="hidden" name="udrtYn" id="udrtYn" value=""/> <!-- 요청 변수 설정 (지하여부) -->
    <input type="hidden" name="buldMnnm" id="buldMnnm" value=""/><!-- 요청 변수 설정 (건물본번) -->
    <input type="hidden" name="buldSlno" id="buldSlno" value=""/><!-- 요청 변수 설정 (건물부번) -->
</form>
</body>
</html>
